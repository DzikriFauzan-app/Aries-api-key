...
(async () => {
  ...
  const innerMock = (llm as any).inner as MockProvider;

  // FIX 1: Hapus maxTokens yang tidak ada di type
  const req = (txt: string): LLMRequest => ({
    prompt: txt,
    temperature: 0
  });
  ...
  try {
    ...
    if (!e.message.includes("Unsafe Prompt")) throw e;
  ...
  innerMock.generate = async () => {
    ...
    } as unknown as LLMResponse; // Force cast aman karena ini mock
  };
  ...
  try {
    ...
    if (!e.message.includes("Unsafe Response")) throw e;
    console.log("   Blocked OK");
  }
  ...
  for (let i = 0; i < 5; i++) {
    try { await llm.generate(req("test")); } catch (e) {} 
  }
  ...
  try {
    ...
    if (!e.message.includes("CircuitBreaker: OPEN")) throw new Error(`Wrong error: ${e.message}`);

...
export class RestorationVault {
  private static verificationState = {
    emailVerified: false,
  ...
  static async initiateRestoration(userId: string, tier: string) {
  ...
  private static handleAIFailure(userId: string) {
  ...
  static verifyManualOverride(inputCode: string, savedCode: string): boolean {
  ...
  static verifyAll(tier: string): boolean {
